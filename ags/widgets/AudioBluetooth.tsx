...
export default function AudioBluetooth() {
  ...
  const micVolText = createBinding(mic, "volume").as(vol => `Mic volume ${percentage(vol)}`)
  const micIcon2Glyph = {
  ...
  >
    <box>
      <MeteringLabel
        class='icon'
        width={29} height={25}
        firstLabel={createBinding(speaker, 'volume_icon').as(icon => audioIcon2Glyph[icon])}
        secondLabel={createBinding(speaker, 'volume_icon').as(icon => audioIcon2Glyph[icon])}
        tooltip_text={audioVolText}
        firstClassName='grey'
        secondClassName={cn('speaker', { muted: createBinding(speaker, 'mute') })()}
        level={createBinding(speaker, 'volume').as(per => linCom(per, 0.29, 0.82))}
      />
      <MeteringLabel
        class='icon'
        width={29} height={25}
        firstLabel={createBinding(mic, 'volume_icon').as(icon => micIcon2Glyph[icon])}
        secondLabel={createBinding(mic, 'volume_icon').as(icon => micIcon2Glyph[icon])}
        tooltip_text={micVolText}
        firstClassName='grey'
        secondClassName={cn('microphone', { muted: createBinding(mic, 'mute') })()}
        level={createBinding(mic, 'volume').as(per => linCom(per, 0.22, 0.83))}
      />
    </box>
  </eventbox>
}
