import { Binding, createComputed } from "ags"
...
/**
  * Example usage
  *
  * cn(['button'], { active: createBinding(mything, 'active') })
  *   -> returns accessor that computes "button active" if the binding resolves to true
  */
export function cn(...classOrBindings: StringOrBooleanBinding[]): () => string {
  ...
  // create an accessor that listens on all of the bindings, and conditionally adds a class based on whether that binding evaluates to true
